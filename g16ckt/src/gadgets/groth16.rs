...
pub fn groth16_verify<C: CircuitContext>(
    ...
    // Verify that all public inputs are valid scalar field elements
    let is_valid_fr = {
    ...
    let finexp = final_exponentiation_montgomery(circuit, &miller_result.f);
    ...
    let tmp2 = circuit.issue_wire();
    let tmp3 = circuit.issue_wire();
